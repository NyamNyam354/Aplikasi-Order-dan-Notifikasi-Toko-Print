...
@section('title', 'Detail Pesanan - PrintShop Admin')

@section('content')
<div class="flex items-center mb-6">
    <a href="{{ route('admin.orders.index') }}" class="mr-3 text-gray-500 hover:text-gray-700">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if($order->status->value === 'pending')
                    <form method="POST" action="{{ route('admin.orders.processing', $order) }}">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                                onclick="return confirm('Mulai memproses pesanan {{ $order->order_number }}?')">
                            Mulai Proses
                        </button>
                    </form>
                ...
                @if($order->status->value === 'processing')
                    <form method="POST" action="{{ route('admin.orders.complete', $order) }}">
                        ...
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm"
                                onclick="return confirm('Selesaikan pesanan {{ $order->order_number }}?')">
                            Selesaikan
                        </button>
                    </form>
                @endif
...
<!-- Cancel Modal -->
<div id="cancel-modal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
        ...
        <form method="POST" action="{{ route('admin.orders.cancel', $order) }}">
            ...
            <div class="mb-4">
                ...
                <textarea id="cancel_reason" name="cancel_reason" rows="3" required minlength="10"
                          ...
                          placeholder="Masukkan alasan pembatalan (minimal 10 karakter)"></textarea>
                @error('cancel_reason')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex justify-end space-x-3">
                <button type="button" onclick="closeCancelModal()" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                    Batal
                </button>
